            ...
            <div class="proposal-form" style="height:100%;">
                ...
                <div class="list-group">
                    @foreach ($publications as $publication)
                        <a href="/edit-publication/{{$publication->id}}" class="list-group-item list-group-item-action">
                            <h4><i>{{$publication->title}}</i></h4>
                            <h4 style="color:#9370DB;">Year:</h4> <h5 style="font-weight:normal">{{$publication->publication_year}}</h5>
                            <h4 style="color:#9370DB;">Author(s): </h4>
                            <h5 style="font-weight:normal">
                                @foreach($pub_authors as $pub_author)
                                    {{$pub_author->title}} {{$pub_author->first_name}} {{$pub_author->last_name}} <br>
                                @endforeach
                            </h5>
                ...
                <div id="proposalForm">
{{--                    @foreach($publications as $publication)--}}
{{--                        <a href="/edit-publication/{{$publication->id}}"><h3>{{$publication->title}}</h3></a>--}}
{{--                        <h4>Year: {{$publication->publication_year}}</h4>--}}

{{--                    @endforeach--}}
{{--                        <h4>Author(s):--}}
{{--                            @foreach($pub_authors as $pub_author)--}}
{{--                                {{$pub_author->first_name}} {{$pub_author->last_name}} <br>--}}
{{--                            @endforeach--}}
{{--                        </h4>--}}
{{--                    {{$pub_authors}}--}}
                        <div class="form-group">
                            <form method="get" action="{{route('newPublication')}}" accept-charset="UTF-8"  autocomplete="off" novalidate="novalidate" enctype="multipart/form-data">
                                <div class="btn-wrapper">
                                <button class="btn btn-success btn-md center-block" type="submit" id="create-proposal-submit" href={{route('newPublication')}}>New Publication</button>
                            </div>
                            </form>
                        </div>
                </div>
...
{{--<h2>Publications:</h2>--}}
{{--@foreach($publications as $publication)--}}
{{--    <a href="/publication-form"><h3>{{$publication->title}}</h3></a>--}}
{{--    <h4>Author: {{$publication->author}}</h4>--}}
{{--    <h4>Year: {{$publication->publication_year}}</h4>--}}
{{--@endforeach--}}


{{--<a href="/publication-form">{{$publications[0]}}</a>--}}
{{--{{$publication[1]}}--}}
